...
export default async function handler(
  ...
  if (req.method === 'POST') {
    const {sended_to,sended_by} = req.body
    const request = await FriendRequest.findOne({where:{sended_by,sended_to}})
    ...
    if(request){
        Friendship.create({
            ...
            res
                .status(201)
                .json({ success: true, message: 'Succesfuly added', user:null });
        })
        ...
        res
            .status(422)
            .json({ success: false, message: 'Request not found', user:null });
    }
